...
* to the pin for interrupt 1 (pin 3 on an arduino uno). 
*
*
...
void setup() {
  ...
  PulseInOne::setup(pulseInComplete);
  
  // use PulseInOne::begin() in place of pulseIn(), and pulseInComplete will fire when a pulse completes.
  PulseInOne::begin();
  /* Calling PulseInOne::begin() will make it watch for a rising edge on a pin indefinitely - there is no timeout.
   * You can stop listening and detach the interrupt by calling PulseInOne::abandon();
   * 
   * After a pulse is detected and the pulseInComplete callback fires you will need to  
   * call PulseInOne::begin() again if you want to watch for another pulse. 
...
void pulseInComplete(unsigned long duration){
  ...
  PulseInOne::begin();  // Start listening again...  
